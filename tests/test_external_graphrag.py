"""Tests for the graphrag adapter (logic via the shared fake client)."""
...
from membench.retrieval.external import graphrag_adapter  # noqa: F401  (registration)
...
def test_maps_results_back_to_item_ids() -> None:
    arm = build_arm("graphrag", client=LexicalIdSearchClient())
...
def test_actionable_error_without_package() -> None:
    arm = build_arm("graphrag")
    with pytest.raises(RuntimeError, match="graphrag"):
        arm.write(_CORPUS)  # type: ignore[attr-defined]
