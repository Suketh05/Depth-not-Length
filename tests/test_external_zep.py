"""Tests for the Zep/Graphiti adapter (logic via the shared fake client)."""
...
from membench.retrieval.external import zep_adapter  # noqa: F401  (registration)
...
def test_registration_is_graph_based_external() -> None:
    spec = get_spec("zep")
    assert spec.capabilities.family is ArmFamily.EXTERNAL
    # Graphiti is graph-based, so unlike the similarity competitors it follows edges.
    assert spec.capabilities.follows_links is True


def test_maps_results_back_to_item_ids() -> None:
    arm = build_arm("zep", client=LexicalIdSearchClient())
...
def test_actionable_error_without_graphiti() -> None:
    arm = build_arm("zep")
    with pytest.raises(RuntimeError, match="graphiti-core"):
        arm.write(_CORPUS)  # type: ignore[attr-defined]
